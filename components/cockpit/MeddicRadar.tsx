...
export const MeddicRadar: React.FC<Props> = ({ assessment, onChangeView }) => {
    if (!assessment) {
        return (
            <div className="h-full flex flex-col items-center justify-center p-6 bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-center min-h-[300px]">
    ...
    return (
        <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm flex flex-col h-full relative overflow-hidden min-h-[350px]">
            ...
            {/* 明确容器高度以修复 Recharts width(-1) height(-1) 错误 */}
            <div className="relative w-full h-[250px] mt-2 -ml-4">
                ...
                <div className="bg-red-50 rounded-lg p-3 border border-red-100 mt-auto">
                    <div className="flex items-start gap-2">
                        ...
                        <div className="text-xs text-red-800">
                            ...
                            <span className="line-clamp-1">{assessment.categories.find(c => c.status === 'Gap')?.missing[0] || "存在关键信息缺失"}</span>
                        </div>
                    </div>
                </div>
            )}
